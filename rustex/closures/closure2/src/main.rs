...
// In earlier examples we were executing simulated_expensive_function
...
// this is value method because it takes self argument
    fn value(&mut self,arg:u32)->u32{
        match self.value{
            ...
            self.value=Some(v);// this line would have also ran even if i wrote
            // like this self.value(v) then it will go in a infinite loop 
            v
        }
    }
...
fn generate_workout(intensity:u32,random_number:u32){
   ...
   // let mut example_closure=|x| x;
    ...
    if intensity<25 {
        ...
        // block
        println!("Do Pushups {}",cached_result.value(intensity));
